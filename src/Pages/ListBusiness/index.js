...
export function getNearestCity(origin, cities) {
    ...
    for (let i = 0; i < cities.length; i++) {
        if (
            typeof cities[i].LatLng.lat !== "number" ||
            typeof cities[i].LatLng.lng !== "number"
        ) {
...
export function sortAlphaAscending(arrayToBeSorted) {
    ...
    for (let i = 0; i < temporaryArray.length; i++) {
        if (typeof temporaryArray[i].name !== "string") {
            console.log(
                "sortAlphaAscending must only accept array of objects where the name object is a string"
            );
            return null;
        }
    }
...
export function sortAlphaDescending(arrayToBeSorted) {
    ...
    for (let i = 0; i < temporaryArray.length; i++) {
        if (typeof temporaryArray[i].name !== "string") {
            console.log(
                "sortAlphaDescending must only accept array of objects where the name object is a string"
            );
            return null;
        }
    }
...
function ListBusiness() {
    ...
    currentBusinesses = virtualBusiness.slice(
    ...
    for (
        let i = 1;
        i <= Math.ceil(virtualBusiness.length / businessesPerPage);
        i++
    ) {
        pageNumbers.push(i);
    }
